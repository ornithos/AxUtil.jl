...
function plot_gmm(dGMM::GMM; figsize=(10,10), alpha=0.5, bins=50, fill=false) where T <: AbstractFloat
    ...
    if d == 2
        ...
        for j in 1:k
            ...
            ax[plottype](levcurv[:,1], levcurv[:,2], alpha=alpha*dGMM.pis[j]/maximum(dGMM.pis))
        end
        return
    end
    ...
    for ix = 1:d, iy = 1:d
        if ix != iy
            ...
            axs[ix, iy][:hist](rsmp[ix, :], bins=bins)
        end
    end
end
...
function _optimise_components_bbb_zygote(d::GMM, log_f::Function, epochs::Int, batch_size_per_cls::Int;
        converge_thrsh::AbstractFloat=0.999, lr::AbstractFloat=1e-3, exitifnan::Bool=false, auto_lr::Bool=true)
    # @debug "(bbb) Input GMM: " dGMM=d
    # (ncomponents(d) > 8) && @debug "(bbb) Input GMM: " dGMM=rmcomponents(d, collect(1:8))
    # (ncomponents(d) > 16) && @debug "(bbb) Input GMM: " dGMM=rmcomponents(d, collect(1:16))
    @warn "Zygote version not created yet."
    n_d = size(d)
    k = ncomponents(d)
    ...
    for ee in 1:epochs
        ...
        for j in 1:k
            ...
            # ===> Calculate entropy of GMM (and gradient thereof)
            Random.seed!(ee_seed)   # common r.v.s (variance reduction)
            _obj, ∇μ, ∇Ls = _gmm_entropy_and_grad(mupars.data, [x.data for x in invLT]; M=batch_size_per_cls, ixs=[j])
            objective += _obj
...
function optimise_components_bbb_revkl(d::GMM, log_f::Function, epochs::Int, batch_size_per_cls::Int;
    ...
    for ee in 1:epochs
        ...
        for c in ixs
            ...
            log_q_terms = reduce(vcat, [llh_unnorm(x .- mus[j,:], invLT[j]) for j in 1:k])
